...
############################################
# Lifecycle (Keep your existing rule)
############################################
...
############################################
# Bucket Policy (CloudTrail, Config, GuardDuty + Deny Unencrypted)
############################################
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      ############################################
      {
        ...
        Action   = "s3:PutObject"
        Resource = "${aws_s3_bucket.this.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/GuardDuty/*"
        Condition = {
          StringEquals = {
            "s3:x-amz-acl" = "bucket-owner-full-control"
          }
